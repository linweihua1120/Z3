...
static void APP_DeInitOtherInterface(void);
  void APP_SetFlashParameter(void);
...
void APP_Bootloader_ProtocolDetection(void)
{
  ...
  uint8_t ucTxDataBuffer[0x20];
  uint32_t uiTxDataLength = 0;
  
   print_int_array(0,"APP_Bootloader_ReadData", ucDataBuffer, 2);

#if 1//获取命令
  APP_Bootloader_ReadData(ucDataBuffer, 2);
 print_int_array(0,"APP_Bootloader_ReadData", ucDataBuffer, 2);
 int plus = (ucDataBuffer[0]<<8 | ucDataBuffer[1]);
sgdSetAndFlash( plus);
return;
  ...
  ucTxDataBuffer[uiTxDataLength++] =   ACK_BYTE  ;
	md5_sign((uint8_t* )(OPTR_BASE ),256,ucTxDataBuffer+uiTxDataLength);
	  print_int_array(0, "md5_sign AES_KEY", ucTxDataBuffer+uiTxDataLength, 16);
	uiTxDataLength+=16;
    break;

  case CMD_AES:
  verification();
// SEGGER_RTT_printf(0, ">>CMD_AES\r\n"  );
//  uint8_t iv[16]={0};
//  uint8_t md5[16]={0};
...
//  	test_aes( );
  ...
  APP_Bootloader_SendData((uint8_t*)0x8008000, 256);
  	 //检查 FLASH_SR 寄存器 BSY 位，确认没有正在进行的 flash 操作
//    if (FLASH_SR_BSY == READ_BIT(FLASH->SR, FLASH_SR_BSY))
//    {
//      APP_Bootloader_SendByte(NACK_BYTE);
...
//      APP_SetFlashParameter();
//    }
//  uint16_t pageNum[1]={0};
//  pageNum[0] = 128;
// SEGGER_RTT_printf(0, ">>CMD_GO	\r\n"  );
//		int res= APP_PageErase((uint16_t *)pageNum, 0, 1);
// SEGGER_RTT_printf(0, " pageNum=%d res = %d\r\n" ,pageNum[0] ,res);
//       if(res==0)
//       	{
...
//
//
//	    APP_WriteFlash(0x8008000, md6, 15);
//  uint8_t buff[16];
//  uint8_t* p = (uint8_t*) 0x8006000;
//  md5_sign(p,6400,buff);
//  print_int_array(0 ,"md5 :",  buff, 16);
//  APP_Bootloader_SendData((uint8_t*)0x1FFF3000, 256);
//    eStatus = APP_Go();
    break;
